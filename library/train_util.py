...
def _worker_auto(captions_chunk, valid_triggers):
...
def _worker(captions_chunk, valid_triggers):
    ...
    for caption in captions_chunk:
        ...
        triggers = [t for t in tags if any(trig in t for trig in valid_triggers)]
        for trig in triggers:
            local_caption_per_tag[trig] += 1
            local_tag_frequency[trig].update(tags)
...
class BaseSubset:
    ...
    def set_tag_frequency(self, captions):
        ...
        if is_automatic:
            self.parallel_count(captions, workers=12, chunk_size=10_000, is_automatic=is_automatic)
        else:
            self.normal_count(captions)
        ...
        if self.adaptive_dropout:
            if not self.dropout_prob:
                ...
                print(self.caption_per_tag)
                print(self.dropout_prob)
                raise RuntimeError(f"Missing dropout probabilities for entry")
            # print random sample of dropout probabilities
            sampled_index = random.sample(range(len(self.trigger_token)), 1)
            for i in sampled_index:
                trigger = self.trigger_token[i]
                # get max probability and its corresponding tag
                max_prob = max(self.dropout_prob[trigger].values())
                max_tag = max(self.dropout_prob[trigger], key=self.dropout_prob[trigger].get)
                min_prob = min(self.dropout_prob[trigger].values())
                mean = sum(self.dropout_prob[trigger].values()) / len(self.dropout_prob[trigger])
                logger.info(f"Trigger: {trigger}, mean_prob: {mean}, min_prob: {min_prob}, max_prob: {max_prob}")
                min_tag = min(self.dropout_prob[trigger], key=self.dropout_prob[trigger].get)
                logger.info(f"Trigger: {trigger}, min_prob: {min_prob}, min_tag: {min_tag}")
                logger.info(f"Trigger: {trigger}, max_prob: {max_prob}, max_tag: {max_tag}")
    def normal_count(self, captions):
        """
    ...
    def parallel_count(self, captions, workers=None, chunk_size=10_000, is_automatic=False):
        ...
        with ProcessPoolExecutor(max_workers=workers) as pool:
            ...
            worker_to_use = _worker_auto if is_automatic else _worker
            future_to_len = {
                pool.submit(worker_to_use, chunk, valid): len(chunk)
    ...
    def calculate_dropout_prob(self):
        ...
        center = 0.85 * (self.max_dropout_rate - self.min_dropout_rate) + self.min_dropout_rate
        if self.trigger_token is None:
            # No trigger token, so we don't need to do anything
            if self.adaptive_dropout:
                raise RuntimeError("Adaptive dropout is enabled but no trigger token is provided.")
            return
        for trigger in self.trigger_token:
            if trigger not in self.caption_per_tag:
                ...
                if total_captions == 0:
                    raise RuntimeError(f"Missing caption count for trigger {trigger}, {self.caption_per_tag}")
                logger.warn(f"Trigger token {trigger} is not in caption_per_tag, using possible entries {possible_entries} instead.")
                for alter_trigger in possible_entries:
                    for tag, count in self.tag_frequency[alter_trigger].items():
                        if "character:" in tag or trigger in tag or alter_trigger in tag:
                            # Skip character: tags
                            continue
                        ...
                        self.dropout_prob[trigger][tag] = max(dropout, self.dropout_prob[trigger].get(tag, 0.0))
            else:
                total_captions = self.caption_per_tag[trigger]
                if total_captions == 0:
                    raise RuntimeError(f"Missing caption count for trigger {trigger}, {self.caption_per_tag}")
